...
namespace OpenCVIFTBridge {
    ...
    // VERSÃO MELHORADA - Gera qualquer quantidade de sementes
    SeedSet generateAutomaticSeeds(const Image& image, int numSeeds) {
        ...
        if (numSeeds <= 0) return seedSet;
        
        std::vector<std::pair<int, int>> positions;
        
        if (numSeeds <= 8) {
            // Para poucos sementes, usa posições fixas otimizadas
            ...
            // Para muitas sementes, usa distribuição em grade
            int seedsPerRow = static_cast<int>(std::sqrt(numSeeds)) + 1;
            int seedsPerCol = (numSeeds + seedsPerRow - 1) / seedsPerRow; // Arredonda para cima
            
            // Calcula espaçamento
            int stepX = std::max(1, width / (seedsPerRow + 1));
            int stepY = std::max(1, height / (seedsPerCol + 1));
            
            // Distribui sementes em grade com pequeno offset para evitar padrões
            for (int i = 0; i < numSeeds; i++) {
                int row = i / seedsPerRow;
                int col = i % seedsPerRow;
                
                int x = stepX * (col + 1);
                int y = stepY * (row + 1);
                
                // Adiciona pequeno offset determinístico para variar posições
                int offsetX = (i * 17) % (stepX / 4 + 1); // Offset baseado no índice
                int offsetY = (i * 23) % (stepY / 4 + 1);
                
                x = std::max(0, std::min(width - 1, x + offsetX - stepX/8));
                y = std::max(0, std::min(height - 1, y + offsetY - stepY/8));
